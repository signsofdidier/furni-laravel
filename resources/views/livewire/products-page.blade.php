                                    ...
                                    @foreach($categories as $category)
                                        <li class="filter-item" wire:key="{{ $category->id }}">
                                            <label class="filter-label">
                                                <input type="checkbox" wire:model.live="selected_categories" value="{{ $category->id }}" class="w-4 h-4 mr-2">
                            ...
                            @foreach($categories as $category)
                                <li class="mb-4" wire:key="{{ $category->id }}">
                                    <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                                        --}}
{{--Value is voor de filter checkboxes--}}{{--

                                        <input type="checkbox" wire:model.live="selected_categories" {{ $category->slug }} value="{{ $category->id }}" class="w-4 h-4 mr-2">
                                        <span class="text-lg">{{ $category->name }}</span>
                                    </label>
                                </li>
                            @endforeach
                            ...
                            @foreach($brands as $brand)
                                <li class="mb-4" wire:key="{{ $brand->id }}">
                                    <label for="{{ $brand->slug }}" class="flex items-center dark:text-gray-300">
                                        <input type="checkbox" wire:model.live="selected_brands" {{ $brand->slug }} value="{{ $brand->id }}" class="w-4 h-4 mr-2">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div>
                            ...
                            <input type="range" wire:model.live="price_range" class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer mt-5" max="5000" value="1" step="1">
                            <div class="flex justify-between ">
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(0, 'EUR') }}</span>
                                <span class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(5000, 'EUR') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{ $product->id }}">
                                <div class="border border-gray-300 dark:border-gray-700">
                                    ...
                                    <div class="p-3 ">
                                        ...
                                        <p class="text-lg ">
                                            <span class="text-green-600 dark:text-green-600">{{ Number::currency($product->price ?? 0, 'EUR') }}</span>
                                        </p>
                                    </div>
...
{{--Add to cart button--}}{{--
                                        ...
                                        <a wire:click.prevent='addToCart({{ $product->id }})' href="#" class="text-gray-500 flex items-center space-x-2 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
                                            ...
                                            --}}
{{--wire:loading="addToCart({{ $product->id }})" toont wat tussen de span staat als je iets aan de cart add--}}{{--

                                            --}}
{{--wire:target="addToCart({{ $product->id }})" zorgt dat enkel de hyidige product geupdate wordt en niet allemaal tegelijk--}}{{--

                                            --}}
{{--wire:loading.remove doet deze zin weg tijdens het laden--}}{{--
